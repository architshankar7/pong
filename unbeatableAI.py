...
#insert if ballX_change/ bally_change is less than or greater than a certain number for optimal ai logic

#INITIALIZE
pygame.init()

# define the RGB value for white,
#  green, blue, red, and orange color .
white = (255, 255, 255)
green = (0, 255, 0)
blue = (21, 36, 173)
red = (255, 0, 13)
...
start = True
#SCREEN (sets pixel size for screen)
...
while running:
    ...
    if playerY <= 0:
        playerY = 0
    #AI LOC AND BEHAVIOR
    if ballY_change >= 1.145648312*ballX_change > 0:
        if ballX < 360 and aiY != 200:
            if aiY > 200:
                aiY -= aiY_change
            elif aiY < 200:
                aiY += aiY_change
        if ballX > 360 and ballY > aiY:
            aiY += aiY_change
        if ballX > 360 and ballY < aiY:
            aiY -= aiY_change
    if ballY_change <= 1.145648312*ballX_change > 0:
    ...
    #COLLISION BEHAVIOR
    if playerX - 20 <= ballX <= playerX + 20:
